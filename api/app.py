...
# --- Chat API Route (Updated with Smart Search) ---
# @app.post("/ask")
# async def ask_bot(request: Request):
#     try:
#         data = await request.json()
...
#         # STEP 1: Smart Check in Supabase (Check for exact or partial matches)
#         db_query = supabase.table("manual_faqs").select("question, answer").execute()
        
#         manual_answer = None
#         if db_query.data:
#             for row in db_query.data:
#                 saved_q = row['question'].lower()
#                 # Agar user ka input aur saved question milte julte hon
#                 if saved_q in user_input or user_input in saved_q:
#                     manual_answer = row['answer']
#                     break
        
#         if manual_answer:
#             return {"answer": manual_answer}
